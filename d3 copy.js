...
const columnsToExclude = [
    "track_artist", "track_id", "track_name",
    "track_album_id", "track_album_name", "track_album_release_date",
    "playlist_name", "playlist_id", "playlist_subgenre", "playlist_genre",
    "key", "loudness", "mode", "instrumentalness", "duration_ms", "valence"
];

dimensions = ["track_popularity", "energy", "tempo", "danceability", "liveness"]
...
for (let i in dimensions) {
    ...
    y[name] = d3.scaleLinear()
        .domain(d3.extent(data, function (d) { return +d[name]; })) // Convert to number
        .range([height, 0]);
}
...
const doNotHighlight = function (event, d) {
    d3.selectAll(".line")
        ...
        .style("stroke", function (d) { return (color(d.track_artist)) })
        .style("opacity", "1")
...
svg
    ...
    .data(data)
    .join("path")
    .attr("class", function (d) { return "line " + d.track_artist.replace(/\s+/g, "_"); }) // Replace spaces with underscores
    .attr("d", path)
    .style("fill", "none")
    .style("stroke", function (d) { return (color(d.track_artist)) })
    .style("opacity", 0.5)
...
const scatterSvg = d3.select("#scatter_plot")
.append("svg")
...
tooltip.html(`${d.track_name} by ${d.track_artist}`)
...
const barSvg = d3.select("#barchart1")
.append("svg")
...
barSvg.selectAll("rect")
    ...
    .on("mouseover", function(event, d) {
        d3.select(this)
            .style("opacity", 1);
        tooltip.style("opacity", 1)
            .html(`Song: ${d.track_name}<br>Duration: ${d.duration_min.toFixed(2)} min<br>Date: ${d3.timeFormat("%Y-%m-%d")(d.track_album_release_date)}`)
            .style("left", (event.pageX + 10) + "px")
            .style("top", (event.pageY) + "px");
    })
...
function showArtistButtons(page) {
    ...
    const endIndex = Math.min(startIndex + artistsPerPage, uniqueArtists.length);
    ...
    // Pagination feature
    const paginationDiv = buttonContainer.append("div")
    ...
    const artistButtonsDiv = buttonContainer.append("div")
        .attr("class", "artist-buttons-grid");
        
    artistButtonsDiv.selectAll("button")
        .data(currentArtists)
        .enter()
        .append("button")
...
d3.csv("/spotify_songs1.csv").then(function(data) {
    ...
    const features = ["danceability", "energy", "valence", "acousticness", "speechiness", "liveness"];
